...
P_c = 5;         % dBm
omega_array = 15:2:50;      % dBm，pdmax的上限
...
% 理论值
K = 1;
max_actr_theo_array1 = zeros(1,numel(omega_array));
for idx = 1:numel(omega_array)
    omega = omega_array(idx);
    P_dmax_array = 0:0.5:omega;
...
figure();
plot(omega_array, max_actr_theo_array1, 'b.-', 'LineWidth', 1.0);
hold on;

% % 仿真值
% max_actr_simu_array = zeros(1,numel(omega_array));
% for idx = 1:numel(omega_array)
%     fprintf("- %d / %d \n", idx, numel(omega_array));
%     omega = omega_array(idx);
%     P_dmax_array = 0:0.5:omega;
...
% end
% plot(omega_array, max_actr_simu_array, 'bs', 'LineWidth', 1.0);
% hold on;
...
% 理论值
K = 2;
max_actr_theo_array2 = zeros(1,numel(omega_array));
for idx = 1:numel(omega_array)
    omega = omega_array(idx);
    P_dmax_array = 0:0.5:omega;
    actr_theo_array = zeros(1,numel(P_dmax_array));
...
plot(omega_array, max_actr_theo_array2, 'r.--', 'LineWidth', 1.0);
hold on;
...
% 理论值
K = 3;
max_actr_theo_array3 = zeros(1,numel(omega_array));
for idx = 1:numel(omega_array)
    omega = omega_array(idx);
    P_dmax_array = 0:0.5:omega;
    actr_theo_array = zeros(1,numel(P_dmax_array));
...
plot(omega_array, max_actr_theo_array3, 'g.-.', 'LineWidth', 1.0);
hold on;
...
%     max_actr_simu_array(idx) = max(actr_simu_array);
% end
% plot(omega_array, max_actr_simu_array, 'rd', 'LineWidth', 1.0);


%%
grid on;
set(gca,'FontName','Times New Roman');      % 设置坐标轴字体
xlabel('The upper bound of $P_d^{\mathrm{max}}$, $\Omega$ (dBm)','Interpreter','latex','FontName','Times New Roman','FontSize',12);

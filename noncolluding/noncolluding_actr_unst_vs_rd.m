...
R_c = 0.5;       % Mbps
R_d_array = 0:0.1:10;       % Mbps

M = 10^4;

% 理论值
actr_theo_array = zeros(1,numel(R_d_array));
...
plot(R_d_array, actr_theo_array, 'r.-', 'LineWidth', 1.0);
hold on;


% 仿真值
actr_simu_array = zeros(1,numel(R_d_array));
for idx = 1:numel(R_d_array)
    R_d = R_d_array(idx);
    sum_co = 0;     % 蜂窝中断总次数
    sum_do = 0;     % D2D中断总次数
...
plot(R_d_array, actr_simu_array, 'rs', 'LineWidth', 1.0);


grid on;
set(gca,'FontName','Times New Roman');      % 设置坐标轴字体
xlabel('Predetermined transmission rate of D2D link, $R_d$ (Mbps)','Interpreter','latex','FontName','Times New Roman','FontSize',12);
ylabel('Average covert rate (Mbps)','Interpreter','latex','FontName','Times New Roman','FontSize',12);
handle = legend("$R_c$="+num2str(R_c)+" Mbps, Theory", ...
                "$R_c$="+num2str(R_c)+" Mbps, Simulation");

...
P_dmax = 20;     % dBm
% 理论值
actr_theo_array_1 = zeros(1,numel(P_c_array));
...
figure();
plot(P_c_array, actr_theo_array_1, 'b.-', 'LineWidth', 1.0);
hold on;


% 仿真值
actr_simu_array_1 = zeros(1,numel(P_c_array));
for idx = 1:numel(P_c_array)
    fprintf("%d / %d \n", idx, numel(P_c_array));
    P_c = P_c_array(idx);
    sum_co = 0;     % 蜂窝中断总次数
...
plot(P_c_array, actr_simu_array_1, 'bs', 'LineWidth', 1.0);
hold on;
...
P_dmax = 30;     % dBm
% 理论值
actr_theo_array_2 = zeros(1,numel(P_c_array));
for idx = 1:numel(P_c_array)
    P_c = P_c_array(idx);
    b = ((2^R_c-1)*dbm2w(sigma2)) / (lambda*dbm2w(P_c));
    B = ((2^R_c-1)*lambda*dbm2w(P_dmax)) / (lambda*dbm2w(P_c));
...
plot(P_c_array, actr_theo_array_2, 'r.--', 'LineWidth', 1.0);
hold on;


% 仿真值
actr_simu_array_2 = zeros(1,numel(P_c_array));
for idx = 1:numel(P_c_array)
    fprintf("%d / %d \n", idx, numel(P_c_array));
    P_c = P_c_array(idx);
    sum_co = 0;     % 蜂窝中断总次数
...
plot(P_c_array, actr_simu_array_2, 'rd', 'LineWidth', 1.0);
...
handle = legend("$P_d^{\mathrm{max}}$=20 dBm, Theory", ...
                "$P_d^{\mathrm{max}}$=20 dBm, Simulation", ...
                "$P_d^{\mathrm{max}}$=30 dBm, Theory", ...
                "$P_d^{\mathrm{max}}$=30 dBm, Simulation");

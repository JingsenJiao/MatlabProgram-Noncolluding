...
P_c = 5;        % dBm
% 理论值
actr_theo_array_1 = zeros(1,numel(epsilon_array));
...
figure();
plot(epsilon_array, actr_theo_array_1, 'b.-', 'LineWidth', 1.0);
hold on;


% 仿真值
actr_simu_array_1 = zeros(1,numel(epsilon_array));
for idx = 1:numel(epsilon_array)
...
plot(epsilon_array, actr_simu_array_1, 'bs', 'LineWidth', 1.0);
hold on;
...
P_c = 10;        % dBm
% 理论值
actr_theo_array_2 = zeros(1,numel(epsilon_array));
for idx = 1:numel(epsilon_array)
    epsilon = epsilon_array(idx);
    b = ((2^R_c-1)*dbm2w(sigma2)) / (lambda*dbm2w(P_c));
    B = ((2^R_c-1)*lambda*dbm2w(P_dmax)) / (lambda*dbm2w(P_c));
...
plot(epsilon_array, actr_theo_array_2, 'r.--', 'LineWidth', 1.0);
hold on;


% 仿真值
actr_simu_array_2 = zeros(1,numel(epsilon_array));
for idx = 1:numel(epsilon_array)
    fprintf("%d / %d \n", idx, numel(epsilon_array));
    epsilon = epsilon_array(idx);
    sum_co = 0;     % 蜂窝中断总次数
...
plot(epsilon_array, actr_simu_array_2, 'rd', 'LineWidth', 1.0);
...
set(handle,'Interpreter','latex','FontName','Times New Roman','FontSize',10,'Location','SouthEast');
